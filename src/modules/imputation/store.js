...
const  modulesURL = '/services/imputations/api/v1';
const  addImpuattionURL = modulesURL+'/add';
...
export const useImputationStore = defineStore('imputation', {
  state: () => ({
    ...
    headerTable: [
      { text: 'Reference', value: 'reference', align: 'start', sortable: true },
  ...
  actions: {
    ...
    async add(payload) {
      try {
        await axios.post(addImpuattionURL, payload)
        .then((response) => {
          if(response.status === 200 ){
            this.dataDetails = response.data;
    ...
    test(){
      console.log("Calling TEST action in imputation from another stor");
    },
    //  modifier une academéie
    async destroy(id) {
      try {
